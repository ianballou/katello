  ...
  module Pulp3
    module Repository
      ...
      class CommitUpload < Pulp3::AbstractAsyncTask
        def plan(repository, smart_proxy, upload_prn, sha256)
          plan_self(:repository_id => repository.id, :smart_proxy_id => smart_proxy.id, :upload_prn => upload_prn, :sha256 => sha256)
        end
        ...
        def invoke_external_task
          ...
          if duplicate_sha_artifact_prn
            uploads_api.delete(input[:upload_prn]) if input[:upload_prn]
            output[:artifact_href] = duplicate_sha_artifact_prn
            ...
            output[:pulp_tasks] = [uploads_api.commit(input[:upload_prn], upload_commit)]
          end
        end
