          ...
          sequence do
            upload_results = concurrence do
              upload_ids.zip(unit_keys).collect do |upload_id, unit_key|
                ...
                import_upload_args = {
                  repository_prn: repository.repository_prn,
          ...
          results.each do |result|
            result[:pulp_tasks]&.each do |task|
              ...
              if details && details.dig('type_id') == 'docker_manifest'
                manifest = ::Katello::DockerManifest.find_by(:pulp_prn => details.dig(:metadata, :prn))
                ...
                manifest = ::Katello::DockerTag.find_by(:pulp_prn => details.dig(:metadata, :prn))
            ...
            if !json_results.size && (result[:content_unit_prn])
              json_results << {:type => 'file'}
            end
          end
          json_results
