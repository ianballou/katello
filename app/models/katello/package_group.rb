module Katello
  class PackageGroup < Katello::Model
    ...
    scoped_search :on => :pulp_prn, :rename => :id, :complete_value => true
    ...
    def package_names
      ...
      group = service_class.new(self.pulp_prn)
    ...
    def content_view_filters
      Katello::ContentViewPackageGroupFilterRule.where(uuid: self.pulp_prn).eager_load(:filter).map(&:filter)
    end
  end
end
