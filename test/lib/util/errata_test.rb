...
module Katello
  class Util::ErrataFilterByPulpHref < ActiveSupport::TestCase
    ...
    def setup
      ...
      @erratum2 = Katello::Erratum.new(:errata_id => "test_errata2", :pulp_prn => "whatevs")
      ...
      @empty_erratum = Katello::Erratum.create(:errata_id => "empty", :pulp_prn => "empty")
    ...
    def test_filter_by_pulp_href_excludes_errata_with_missing_module_streams
      ...
      assert_equal [@erratum], filter_errata_by_pulp_href([@modular_erratum, @erratum],
                                                          [@fedora_one.pulp_prn, @fedora_two.pulp_prn, @modular_rpm.pulp_prn],
    ...
    def test_filter_by_pulp_href_includes_errata_with_proper_module_streams
      ...
      assert_equal [@erratum, @modular_erratum].sort, filter_errata_by_pulp_href([@modular_erratum, @erratum],
                                                          [@fedora_one.pulp_prn, @fedora_two.pulp_prn, @modular_rpm.pulp_prn, @module_stream.pulp_prn],
                                                          [@fedora_one.filename, @fedora_two.filename, @modular_erratum_package.filename],
                                                          [@module_stream.module_spec])
    end

    def test_filter_by_pulp_prn_returns_nothing_with_empty_list_of_pulp_prns
      assert_empty filter_errata_by_pulp_href([@erratum, @erratum2], [], [@fedora_one.filename, @fedora_two.filename], [])
    end

    def test_filter_by_pulp_prn_returns_nothing_with_empty_list_of_errata
      assert_empty filter_errata_by_pulp_href([], [@fedora_one.pulp_prn], [@fedora_one.filename, @fedora_two.filename], [])
    end

    # "Proper" errata here means errata with no RPMs that exist in its source repo and any empty errata.
    def test_filter_by_pulp_prn_returns_proper_errata_if_no_matching_packages
    ...
    def test_filter_by_pulp_prn_returns_proper_errata_with_some_matching_packages
      assert_equal [@erratum2, @empty_erratum].sort, filter_errata_by_pulp_href([@erratum, @erratum2, @empty_erratum], [@fedora_one.pulp_prn], [@fedora_one.filename, @fedora_two.filename], []).sort
    end

    def test_filter_by_pulp_prn_identifies_errata_with_all_matching_packages
      assert_equal [@erratum, @erratum2, @empty_erratum].sort, filter_errata_by_pulp_href([@erratum, @erratum2, @empty_erratum],
        [@fedora_one.pulp_prn, @fedora_two.pulp_prn],
        [@fedora_one.filename, @fedora_two.filename], []).sort
    end

    def test_filter_by_pulp_prn_includes_errata_with_missing_packages_not_in_source_repo
