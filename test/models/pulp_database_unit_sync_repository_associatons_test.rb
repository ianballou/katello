...
module Katello
  class PulpDatabaseUnitSyncRepositoryAssociationsTest < ActiveSupport::TestCase
    ...
    def test_non_generic_content_type_associates_new_ids_and_removes_missing_ids_in_map
      ...
      [@rpm, @rpm2].each { |rpm| tracker.push({pulp_href: rpm.pulp_prn}.with_indifferent_access) }
    ...
    def test_generic_different_content_types_associates_new_ids_and_removes_missing_ids_in_map
      ...
      @gcu = Katello::GenericContentUnit.create(name: "one", pulp_prn: "one-uuid", content_type: "first")
      @gcu2 = Katello::GenericContentUnit.create(name: "two", pulp_prn: "two-uuid", content_type: "second")
      @gcu3 = Katello::GenericContentUnit.create(name: "three", pulp_prn: "three-uuid", content_type: "third")

      @repo.generic_content_units = [@gcu, @gcu2]

      indexer = Katello::ContentUnitIndexer.new(content_type: @content_type, repository: @repo)
      tracker = Katello::ContentUnitIndexer::RepoAssociationTracker.new(@content_type, @service_class, @repo)
      tracker.push({pulp_href: @gcu.pulp_prn}.with_indifferent_access)
      tracker.push({pulp_href: @gcu2.pulp_prn}.with_indifferent_access)
