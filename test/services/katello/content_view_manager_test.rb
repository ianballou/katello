...
module Katello
  class ContentViewManagerTest < ActiveSupport::TestCase
    test 'auto_publish_composites!' do
      ...
      composite.expects(:build_auto_publish_request).returns(request)
      request.expects(:content_view_version=).with(cvv)
    ...
    test 'auto_publish_composites! request exists' do
      ...
      composite.expects(:build_auto_publish_request).returns(request)
      request.expects(:content_view_version=).with(cvv)
      request.expects(:save!).raises(ActiveRecord::RecordNotUnique)
